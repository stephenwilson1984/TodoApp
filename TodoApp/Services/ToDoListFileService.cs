...
{
    // This is a hard-coded path to the file. It may not be available on every platform. In your real world
    // app you may want to make this configurable.
    private static readonly string JsonFileName = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "TodoApp", "MyToDoList.txt");
    ...
    public static async Task SaveToFileAsync(IEnumerable<ToDoItem> itemsToSave)
    {
        ...
        // We use a FileStream to write all items to disk
